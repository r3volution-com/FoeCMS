...
if (isset($inadm)) {
	...
	if (isset($_POST["id"]) && $_POST["id"]) {
		...
		if ($_POST["cat"] != $row["category_id"]) {
			mysql_query("UPDATE ".$MYSQL_PREFIX."category SET parent_category_id='".$_POST["cat"]."' WHERE category_id='".$_POST["id"]."'") or die(mysql_error());
		}
		echo "<script>alert('done!');location.href='./index.php'</script>";
	 } else {
?>
<b><?php echo _l("acp_editcategory"); ?></b> <br />
<?php $res = mysql_query("SELECT * FROM ".$MYSQL_PREFIX."category") or die(mysql_error());
while ($row = mysql_fetch_array($res)) { ?>
<?php echo $row["category_id"]." - ".$row["name"]."<a href='index.php?r=".$_GET["r"]."&d=".$row["category_id"]."'> <b>X</b></a><br>"; ?>
