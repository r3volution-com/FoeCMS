<?php 
	include("../config.php");
	include("../common.php");
	if (isset($_GET["f"]) && $_GET["f"] && is_int($_GET["f"]) && CheckLogin()) {
		...
		if ($user_row2["id"] != $USER_ID) {
			...
			if ($row == 0) mysql_query("INSERT INTO ".$MYSQL_PREFIX."friend (user_send, user_receive) VALUES ('".$user_row["id"]."','".$user_row2["id"]."');") or die(mysql_error());
			else {
				$row = mysql_fetch_array($res);
				if ($row["alive"] != 0 && $row["alive"] != 2) mysql_query("UPDATE ".$MYSQL_PREFIX."friend SET alive=2 WHERE (user_send='".$user_row2["id"]."' AND user_receive='".$user_row["id"]."') OR (user_send='".$user_row["id"]."' AND user_receive='".$user_row2["id"]."')") or die(mysql_error());
				else mysql_query("UPDATE ".$MYSQL_PREFIX."friend SET alive=1 WHERE (user_send='".$user_row2["id"]."' AND user_receive='".$user_row["id"]."') OR (user_send='".$user_row["id"]."' AND user_receive='".$user_row2["id"]."')") or die(mysql_error());
			}
			die("<script>location.href='./ucpfriend.php?u=".$_GET["f"]."'</script>");
		} else die("<script>location.href='../msg.php?e=uset_alreadyfriend'</script>");
	} else if (isset($_GET["nf"]) && $_GET["nf"] && is_int($_GET["nf"]) && CheckLogin()) {
		...
		} else die("<script>location.href='../msg.php?e=uset_notfriend'</script>");
	} else if (checkLogin()) {
